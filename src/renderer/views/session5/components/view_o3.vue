    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <span>
            <h1>用户集群智能集成<el-tooltip content="展示了每一类用户的属性重要性占比，用于判断单属性的集群分析" placement="right" effect="light">
                <el-icon>
      ...
      <div style="width: 70%;float: left;">
        <el-scrollbar height="600px">
          <v-chart id="clusterModel" autoresize style="height: 550px; width: 100%;"></v-chart>
        </el-scrollbar>
      </div>
      <div style="width:30%; float: right;">
        <el-card>
          ...
          <el-scrollbar height="440px">
            <el-form>
              <el-form-item label="集群1：">
                <el-select v-model="cluster.name1" class="m-2" placeholder="Select">
                  <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="集群2：">
                <el-select v-model="cluster.name2" class="m-2" placeholder="Select">
                  <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="集群3：">
                <el-select v-model="cluster.name3" class="m-2" placeholder="Select">
                  <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="集群4：">
                <el-select v-model="cluster.name4" class="m-2" placeholder="Select">
                  <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label-width="70px">
                <el-button type="info" plain @click="nameChange">确认修改</el-button>
                <el-button type="primary" @click="AiChange">智能匹配</el-button>
              </el-form-item>
            </el-form>
            <p style="line-height: 150%;">
              匹配规则：<br>
              智能匹配会根据用户每一项行为的重要性，为期排序从而选出高价值用户、大众型用户、潜力型用户和低价值用户
            </p>
          </el-scrollbar>
        </el-card>
      </div>
  ...
  proxy.$axios
    ...
    .then(function (res) {
      if (res.data.code === 200) {
        ...
        var chartDom = document.getElementById('clusterModel')
        ...
        myChart.setOption(option_column)
